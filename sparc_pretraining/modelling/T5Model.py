...
class T5Gen_Model(nn.Module):
    def __init__(self, model_path, tokenizer, special_token_list, dropout, add_special_decoder_token, is_training):
        ...
        self.sos_r_token_id, self.eos_r_token_id, self.sos_q_token_id, self.eos_q_token_id = \
            self.tokenizer.convert_tokens_to_ids(['<_PAD_>', '<sos_b>', '<eos_b>', '<sos_a>', '<eos_a>',
                                                  '<sos_r>', '<eos_r>', '<sos_q>', '<eos_q>'])
        if is_training:
    ...
    def batch_generate(self, src_input, src_mask, generate_mode, max_decode_len):
        ...
        if self.add_special_decoder_token:
            ...
            elif generate_mode == 'sqlg':
                start_token, end_token, start_token_id, end_token_id = '<sos_q>', '<eos_q>', self.sos_q_token_id, self.eos_q_token_id
            else:
