...
def parse_config():
    ...
    parser.add_argument('--use_rcg', type=str, default='True', help='whether using NL2RC data during pretraining.')
    parser.add_argument('--use_twg', type=str, default='True', help='whether using NL2TW data during pretraining.')
    ...
    parser.add_argument('--tokenizer_prefix_path', type=str, help='the path where the tokenizer is stored.')
...
if __name__ == '__main__':
    ...
    preprocessed_tokenizer_path = args.tokenizer_prefix_path
    ...
    # model = T5Gen_Model(args.model_name, data.tokenizer, special_token_list, dropout=args.dropout,
    #                     add_special_decoder_token=True, is_training=True)

    if args.pretrained_path != 'None':
        model = T5Gen_Model(args.pretrained_path, data.tokenizer, special_token_list, dropout=args.dropout,
                            add_special_decoder_token=True, is_training=True)
    else:
        model = T5Gen_Model(args.model_name, data.tokenizer, special_token_list, dropout=args.dropout,
                            add_special_decoder_token=True, is_training=True)
    ...
    print('total steps: ' + str(actual_total_steps))
